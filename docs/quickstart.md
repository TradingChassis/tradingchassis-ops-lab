...
Install, run checks, prepare sample data, run backtest + paper smoke paths, generate backtest-vs-paper evidence, and bring up the local observability stack.
...
After one local backtest run and one local paper run, generate an artifact-backed operational comparison:
...
Use this artifact to review shared operational facts, expected mode differences, and known gaps. It is not strategy performance analytics and does not provide PnL/Sharpe/returns conclusions.
...
MkDocs is not included in the default dev install. To build the site locally:
...
Contributors may install both extras together: `pip install -e ".[dev,docs]"`.
